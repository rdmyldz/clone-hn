...
func (s *SqliteHN) CreatePost(p *models.Post) (int, error) {
	...
	res, err := stmt.Exec(p.Link, p.Title, p.Domain, p.Owner, p.Points, p.ParentID, p.MainPostID, p.CommentNum, time.Now())
	if err != nil {
		return -1, err
	}
...
func (s *SqliteHN) GetPosts(query string) ([]models.Post, error) {
	...
	for rows.Next() {
		var p models.Post
		err := rows.Scan(&p.ID, &p.Link, &p.Title, &p.Domain, &p.Owner, &p.Points, &p.ParentID, &p.MainPostID, &p.CommentNum, &p.CreatedAt)
		if err != nil {
			return nil, err
		}
		ret = append(ret, p)
...
func (s *SqliteHN) GetPost(query, id string) (*models.Post, error) {
	...
	var p models.Post
	err := row.Scan(&p.ID, &p.Link, &p.Title, &p.Domain, &p.Owner, &p.Points, &p.ParentID, &p.MainPostID, &p.CommentNum, &p.CreatedAt)
	if err != nil {
		return nil, err
	}
